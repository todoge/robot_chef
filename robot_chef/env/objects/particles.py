...
def spawn_particles(
    ...
    mass: float = 0.002,
    friction: float = 0.4,
    color: tuple[float, float, float, float] = (0.96, 0.94, 0.84, 1.0),
) -> ParticleSet:
    """Spawn spherical particles inside a pan volume."""
    ...
    max_spawn = min(center[2] + max(spawn_height, radius * 3), center[2] + pan_height - radius)
    for _ in range(count):
        r = pan_radius * math.sqrt(rng.random())
