...
def create_rice_bowl(
    ...
    collision_orientations = [[0.0, 0.0, 0.0, 1.0]] * 5

    collision = p.createCollisionShapeArray(
        shapeTypes=shape_types, radii=radii, halfExtents=half_extents, lengths=lengths,
        collisionFramePositions=collision_positions, collisionFrameOrientations=collision_orientations,
        physicsClientId=client_id,
    )
    rgba_base = [0.95, 0.95, 0.95, 1.0]
    visual = p.createVisualShapeArray(
        shapeTypes=shape_types, radii=radii, halfExtents=half_extents, lengths=lengths,
        rgbaColors=[rgba_base] * 5, visualFramePositions=collision_positions,
        visualFrameOrientations=collision_orientations, physicsClientId=client_id,
    )

    body_id = p.createMultiBody(
        baseMass=mass, baseCollisionShapeIndex=collision, baseVisualShapeIndex=visual,
        basePosition=base_position, baseOrientation=orientation, physicsClientId=client_id,
    )
    p.changeDynamics(
    ...
    properties = {
        "radius": radius, "inner_radius": inner_radius, "inner_height": inner_height,
        "base_thickness": base_thickness, "spawn_height": inner_height * 0.7, "mass": mass,
    }
